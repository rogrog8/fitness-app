export class UI {
    ...
    renderUI(todayData, userProfile) {
        ...
        todayData.forEach(item => {
            ...
            this.clearAllBtn.classList.remove('hidden'); // <-- TAMPILKAN TOMBOL
            // ... (sisa logika render riwayat dan chart sama)
            } else {
            this.dashboardSection.classList.add('hidden');
            this.clearAllBtn.classList.add('hidden'); // <-- SEMBUNYIKAN TOMBOL
        }
    ...
    renderFoodList(foods, target, title = null) {
        const container = target === 'local' ? this.foodListLocalEl : this.foodListApiEl;
            ...
            foods.forEach(food => {
                ...
                foodItem.innerHTML = `
                    <div class="food-item-info">
                        <strong style="text-transform: capitalize;">${food.name}</strong>
                        <span>${Math.round(food.calories)} kcal per ${food.serving}</span>
                    </div>
                    <button class="add-food-btn" data-name='${JSON.stringify(food)}'>+</button>
                `;
        ...
        this.dailyChart = new Chart(this.dailyChartCanvas, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: data,
                    backgroundColor: this.calorieColors,
                    borderColor: '#FFFFFF',
                    borderWidth: 2,
                    hoverOffset: 8
                }]
            ...
            this.dailyChart.data.labels.forEach((label, i) => {
                ...
                legendHTML += `
                    <div class="legend-item">
                        ...
                        <span class="legend-value">${value} kcal</span>
                    </div>
                `;
            });
        }
